...
def test_snapshot_advertises_checkpoint_evaluation_route(
    ...
    assert capabilities["persistentStateTraining"] is True
    assert capabilities["randomOffsetAuxiliary"] is False
    assert capabilities["randomOffsetAuxiliaryScope"] is False
        ...
        json.dumps(
            {
                ...
                "configuration": {
                    ...
                    "stateLanes": 1, "trainingShardTokens": 2_048,
                    "randomOffsetAuxiliaryWeight": 0.25,
                    "randomOffsetAuxiliaryScope": "full_corpus",
                },
            }
        ),
        encoding="utf-8",
...
def test_curriculum_breadth_requires_append_only_lanes_and_never_shrinks(
    ...
    with pytest.raises(ValueError, match="disposable cold-context"):
        laboratory.continue_run(
            ContinueSpec(
    ...
    with pytest.raises(ValueError, match="auxiliary scope"):
        laboratory.continue_run(
            ContinueSpec(
                "trial", "GPU-example", random_offset_auxiliary_scope="unknown",
                structure=False, topology_profile="fixed",
            )
        )
...
def test_laboratory_records_continuous_experience_or_cold_control(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="state lanes"):
        laboratory._validate_spec(
            LaunchSpec("invalid", "GPU-example", state_lanes=513)
        )
    laboratory._validate_spec(
        LaunchSpec("valid", "GPU-example", state_lanes=512)
    )
